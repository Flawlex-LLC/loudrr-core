...
async function proxyRequest(
  ...
  const backendUrl = `${BACKEND_URL}/api/loud/${path}/${url.search}`;

  console.log(`[Loud Proxy] ${request.method} ${backendUrl}`);
  ...
  try {
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      console.error(`[Loud Proxy] Backend returned non-JSON (${response.status}):`, text.substring(0, 200));
    ...
    console.error('[Loud Proxy] Connection error:', error);
    return NextResponse.json(
