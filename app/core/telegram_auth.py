...
def verify_init_data(init_data: str, bot_token: str) -> dict:
    ...
    recieved_hash = pairs.pop("hash", None)
    if recieved_hash is None:
        raise ValueError("Missing hash in init data")
    
    data_check_string = "\n".join(f"{k}={v}"for k, v in sorted(pairs.items()))
    secret_key = hmac.new(b"WebApp", bot_token.encode(), hashlib.sha256).digest()

    check_hash = hmac.new(secret_key, data_check_string.encode(), hashlib.sha256).hexdigest()

    if check_hash != recieved_hash:
